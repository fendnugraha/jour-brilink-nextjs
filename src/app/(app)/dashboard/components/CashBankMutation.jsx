...
import { ArrowLongRightIcon, PlusCircleIcon } from '@heroicons/react/24/solid'
...
const CashBankMutation = ({ warehouse, warehouses }) => {
    ...
    const mutationInSum = acc_id => {
        return journalsByWarehouse?.data?.reduce(
            (sum, journal) => (journal.debt_code === acc_id && journal.trx_type === 'Mutasi Kas' ? sum + journal.amount : sum),
            0,
        )
    }
    ...
    const mutationOutSum = acc_id => {
        return journalsByWarehouse.data?.reduce(
            (sum, journal) => (journal.cred_code === acc_id && journal.trx_type === 'Mutasi Kas' ? sum + journal.amount : sum),
            0,
        )
    }
            ...
            <div className="mb-4 bg-white overflow-hidden shadow-sm sm:rounded-2xl">
                ...
                <table className="table w-full">
                    <thead>
                        <tr>
                            <th>
                                Dari <ArrowLongRightIcon className="size-5 inline" /> Ke
                            </th>
                            <th>Jumlah</th>
                        </tr>
                    </thead>
                        ...
                        {currentItems.map((journal, index) => (
                            <tr key={index}>
                                <td className="">
                                    ...
                                    {journal.cred.acc_name} <ArrowLongRightIcon className="size-5 inline" /> {journal.debt.acc_name}
                                </td>
